...
function preload()
{
	...
	for(let x=0; x<50; x++)
	{
		animacionZombieCono[x]=loadImage(`img/Zombies/Zombie Cono/${x}.png`);
	}
...
function definirNivel()
{
	...
	{
		case 1:
			cantidadZombiesMatar=10
			noZombies=4
		break

		case 2:
			cantidadZombiesMatar=20
			noZombies=8
		break
...
function cargarSol()
{
	solImg={
		...
		posX: Math.floor(random(200,900)),
...
{
	let posXzombie=Math.floor(random(1350,1750))
	let posYzombie
...
function inicializarPlanta(posX, posY, cartaNombre, cartaCosto)
{
	...
	if(!cuadroOcupado)
	{
		...
		let planta={
			...
			tiempoMostrarSol: 1200,
			tiempoMostrarSolAux: 1200,
			mostrarSol: false,
			psfx: false,
		}
